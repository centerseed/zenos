...
Approved，2026-04-23；Wave 9 runtime migration 於 2026-04-24 落地。統合本次 Grand Ontology Refactor 的 9 個 Wave，取代（或部分取代）上述 9+ 份 ADR。
...
本 ADR **不是新的架構決策**，是對現存分散決策的**最終收斂聲明**，配合 `refactor-index.md`（Wave 0-9 master index）+ 八份 canonical SPEC 落地（見 §2）。
...
### 2. 鎖定八份 canonical SPEC

> 註：表中 `SPEC-task-governance` 與 `SPEC-doc-governance` 目前 frontmatter 仍為 **Under Review**。本 ADR 指定其為目標 canonical，待 Wave 9 Phase G 與 data-model-consolidation 對應 migration 落地後同步升 Approved。在此之前，caller 對 L3-Action / L3-Document runtime 行為仍以既有實作與 MCP contract 為準。
...
主 SPEC v2 §9 的 **L3-Action subclass table**（`entity_l3_milestone / plan / task / subtask`）已於 Wave 9 落地為 runtime canonical。`zenos.tasks` / `zenos.plans` / `task_entities` / `task_blockers` 已 drop；caller 仍以 `SPEC-task-governance` 的 MCP/API contract 為準。

ADR-028（plan-primitive）、ADR-044（task-ownership）、ADR-047（L1 level SSOT）已由本 ADR + 主 SPEC v2 完全 supersede。
...
**保留 Approved（非 Superseded），等待後續 data-model consolidation 落地**：

- **ADR-046 document-entity-boundary**：spec 目標態已寫在主 SPEC v2 §8.1 + `SPEC-doc-governance`，但 runtime 尚未完成——`src/zenos/domain/knowledge/models.py:88` 仍有獨立 `Document` dataclass；`src/zenos/domain/governance.py:223-230` 治理邏輯明確雙路徑（Document 與 Entity 並行）；`zenos.documents` 仍活著。收斂由 `PLAN-data-model-consolidation` 承擔。
...
- **runtime 對齊**：L3-Action runtime 已對齊主 SPEC §9 subclass DDL；剩餘 Document / Protocol 收斂由各自 PLAN 承擔。
- **Wave 9 migration 已完成**：主 SPEC §9 subclass DDL + governance canonical 已落到 prod；legacy action tables 已 drop。

### 負面

- 歷史 ADR 的設計脈絡分散在各 supersede note；要完整理解某個決策演進仍需順著 supersede chain 走
- 已落地後，舊 ADR 仍保留大量歷史語境；閱讀時需以本 ADR frontmatter `supersedes` 與主 SPEC 為準。

### 後續處理

- Wave 9（code + migration）已於 2026-04-24 完成；後續只保留 API-shape cleanup 與 `PLAN-data-model-consolidation` 相關範圍。
- 本 ADR 的 supersede 清單是**終態清單**，未來新 ADR 若要推翻本次決策應明確 supersede ADR-048 而非個別舊 ADR

## Implementation

本 ADR 不帶新的實作步驟——它是**已完成的 Wave 0-9 refactor 的終態聲明**：
