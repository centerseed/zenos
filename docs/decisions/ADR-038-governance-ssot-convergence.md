...
- 若 governance_rules.py 與 SPEC 文字不同步，比現況更糟（spec 說一套、server 回一套）→ 由 CI lint + SSOT library tests 把關；不再依賴 production runtime `analyze(check_type="governance_ssot")`
...
## Post-Decision Note (2026-04-18)

實作落地時確認：server runtime 不應讀取 repo filesystem 來執行 governance SSOT audit。

- `governance_ssot_audit.py` 保留為 library / CI lint 使用
- `analyze(check_type="governance_ssot")` 已自 runtime 移除
- SSOT 漂移改由 `scripts/lint_governance_ssot.py`、pytest 與 PR gate 處理
