---
type: SPEC
id: SPEC-governance-observability
status: Under Review
ontology_entity: governance-framework
created: 2026-03-25
updated: 2026-04-23
depends_on: SPEC-governance-framework, SPEC-ontology-architecture v2 §7, SPEC-doc-governance, SPEC-mcp-tool-contract
---
...
> 不透過 governance_guide 暴露給外部 agent——這是 ZenOS 飛輪的數據基礎。
> 部分能力（推斷準確度趨勢、健康度指標）可透過 Dashboard 對用戶可見，但演算法細節不對外。
> 框架歸屬見 `SPEC-governance-framework` 治理功能索引。

**版本：** 2.1（2026-04-23 — 補 frontmatter、`SPEC-l2-entity-redefinition` / `SPEC-document-bundle` 舊引用改為主 SPEC §7 / `SPEC-doc-governance`）
**作者：** PM
**相關文件：** `docs/archive/specs/SPEC-governance-quality.md`、`SPEC-ontology-architecture v2 §7`（L2 canonical）、`SPEC-doc-governance`（L3-Document canonical，含舊 bundle 內容）
...
2026-04-10 起 Gemini API 故障 7 天期間，觀察到 L3 document bundle 路徑部分退化。回溯發現 server 端有隱式 LLM 依賴進入 bundle 主路徑，違反 `SPEC-doc-governance §3`（2026-04-23 起 canonical；舊 `SPEC-document-bundle` 已併入）的「server 端不得依賴 LLM」規範。本章定義持續性的 server 端 LLM 依賴健檢機制，避免類似問題再次發生。
...
- `bundle_highlights_coverage` 門檻沿用 `SPEC-doc-governance §3.4`（2026-04-23 起 canonical；舊 `SPEC-document-bundle` P0-14 已併入）：
  - `>= 80%` → green
  - `50% ~ 80%` → yellow
  - `< 50%` → red
...
- `governance_ssot` 漂移已改為 CI-only lint，不再併入 runtime `health_signal`
- production health KPI 只反映線上可觀測訊號，不承擔 repo-contract drift 檢查
