...
ZenOS 的 ontology 有多層治理：L2 知識節點、L3 文件、Task（Action Layer）。每一層都有自己的治理 spec，定義品質門檻、生命週期、關聯規則。
...
- `SPEC-l2-entity-redefinition`（L2 知識節點治理）
- `SPEC-doc-governance`（L3 文件治理）
- `SPEC-task-governance`（Task 治理）
- 未來新增的任何治理 spec
...
**注意**：前兩欄（L2、L3 文件）屬於 entity 分層；Task 不屬於任何 entity 層，它是獨立的 collection，在此列入是為對比治理結構的共通性，不代表 Task 是 entity 的子類型。

| 維度 | L2 知識節點 | L3 文件 | Task（Action Layer） |
|------|-----------|---------|---------------------|
| **權威 Spec** | `SPEC-l2-entity-redefinition` | `SPEC-doc-governance` | `SPEC-task-governance` |
| **對象類型** | entity（L2 層，公司共識概念；module 是常見型態，不是唯一型態） | document entity | task（獨立 collection，非 entity） |
| **品質門檻** | 三問全過、summary 跨角色可讀、至少 1 條具體 impacts | frontmatter 必填、分類正確、ontology_entity 有值 | title 動詞開頭、AC 2-5 條、linked_entities 1-3 個、description 含背景/問題/期望 |
| **生命週期** | draft → confirmed → stale | Draft → Under Review → Approved → Superseded → Archived | backlog → todo → in_progress → review → done → archived（+ blocked 中間態） |
| **關聯規則** | 至少 1 條具體 impacts 路徑 | parent_id 掛載 + source.uri + ontology_entity | linked_entities 1-3 個（Type A/B/C），至少掛 1 個主要治理節點 |
| **反饋路徑** | impacts 變動觸發下游檢查 | supersede/archive 時更新追溯關係 + ontology sync | 完成後反寫 document / blindspot / entity；知識反饋未完成不得通過 |
| **衝突仲裁** | 定義三問 + impacts gate；不覆寫 L3 或 Task 規則 | 定義文件生命週期；不覆寫 L2 升降級規則 | 定義建票品質與驗收；不覆寫 L2 或 L3 規則 |
...
| L2 三問判斷標準 | External | `SPEC-l2-entity-redefinition` | 三問的文字定義與判斷流程 |
| 四維標籤結構 | External | `SPEC-ontology-architecture` | what/why/how/who 的定義與用法 |
| 命名規範與格式要求 | External | `SPEC-l2-entity-redefinition` | 命名限制、長度、禁止模式 |
| 關係類型定義 | External | `SPEC-l2-entity-redefinition` | impacts/enables/depends_on ��語意 |
| L2 生命週期狀態機 | External | `SPEC-l2-entity-redefinition` | draft→confirmed→stale 及轉換條件 |
| 可調整 vs 不可調整參數 | External | `SPEC-l2-entity-redefinition` | server hardcoded 與 user adjustable 邊界 |
...
| `SPEC-l2-entity-redefinition` | 本框架的實例——L2 治理規則 | Base Layer |
