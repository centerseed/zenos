...
CRM 模組以 ZenOS 為基底建立——客戶公司、聯絡人橋接為 ZenOS L1 entity，讓 CRM 資料進入知識圖譜，實現跨模組關聯查詢，同時驗證 ZenOS 多用戶治理、AI 擷取、關聯查詢三項核心能力。
...
#### 公司管理
- **描述**：建立、查看、編輯客戶公司資料。每家公司對應一個 ZenOS L1 entity。
...
#### 聯絡人管理
- **描述**：為公司新增聯絡人（窗口）。每個聯絡人對應一個 ZenOS L1 entity，並關聯至所屬公司。
- **欄位**：姓名、職稱、Email、電話、備忘
- **Acceptance Criteria**：
  - Given 用戶新增聯絡人並選擇所屬公司，When 儲存，Then 聯絡人出現在公司下方，ZenOS L1 entity（type: person）建立，並建立與公司 entity 的關聯
...
- **ZenOS L1 橋接**：新增/修改公司、聯絡人時，必須同步建立或更新 ZenOS L1 entity（`mcp__zenos__write`）；橋接為非同步可接受，但最終一致性必須保證
...
- **商機是否需要橋接為 ZenOS entity 或 Task？** 目前 spec 以 CRM 自己的資料表管理商機，僅公司/聯絡人橋接 L1 entity。若需要在知識地圖上看到商機節點，Architect 需評估是否把商機也寫入 ZenOS entities。
