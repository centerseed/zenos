...
- ~~**商機是否需要橋接為 ZenOS entity 或 Task？**~~ → **已決定（SPEC-crm-intelligence）**：建立 deal 時自動橋接為 ZenOS entity（type: deal），掛在公司 entity 之下。AI Intelligence 的 briefing/debrief 產出需要掛載在 deal entity 上。
- ~~**ZenOS governance 三問**~~ → **已決定（SPEC-crm-intelligence）**：P0 不觸發治理 pipeline，P2 加結案回顧（Deal Retrospective）。
- ~~**Dashboard tab 命名**~~ → **已決定**：維持「客戶」。「業務」有歧義。
