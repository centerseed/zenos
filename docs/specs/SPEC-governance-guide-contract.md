...
4. 定義治理規則 SSOT 的稽核機制（`analyze(check_type="governance_ssot")`）
...
Server 端必須支援以下 topic，每個 topic 至少要有 level 1 / 2 兩個層次的內容：

| topic | 對應的人讀 spec | 覆蓋的治理層 |
|-------|----------------|-------------|
| `entity` | `SPEC-l2-entity-redefinition` | L2 知識節點 |
| `document` | `SPEC-doc-governance` | L3 文件基礎治理 |
| `bundle` | `SPEC-document-bundle` | L3 文件 bundle-first 與路由規則 |
| `task` | `SPEC-task-governance` | Task（Action Layer） |
| `capture` | `skills/release/zenos-capture/SKILL.md` 的流程骨架 | Capture / Sync 流程 |
| `sync` | 同上 | 增量同步流程 |
| `remediation` | `SPEC-governance-feedback-loop` | 治理缺口修復流程 |

> 這個清單**可擴充但不可縮減**。新增治理模組（例如 Plan 治理）時必須同步新增 topic。
...
- `AC-P0-1-2` Given agent 呼叫 `governance_guide(topic="unknown_topic")`，When server 處理，Then 回傳 `status="rejected"`，`data.error.code="UNKNOWN_TOPIC"`，`data.error.available_topics` 列出所有合法 topic
...
- `AC-P0-2-3` Given agent 呼叫 `governance_guide(topic="task", level=99)`，When server 處理，Then 回傳 `status="rejected"`，`data.error.code="INVALID_LEVEL"`
...
依 ADR-038 D4，`SPEC-*-governance.md` 與 `SPEC-document-bundle` / `SPEC-mcp-tool-contract` 等治理相關 spec 修訂時：
...
### P0-6: SSOT 稽核 — `analyze(check_type="governance_ssot")`

新增 analyze check 偵測 SSOT 不同步：
    ...
    "findings": [
      {
        ...
        "topic": "bundle",
        "spec": "SPEC-document-bundle",
        "diff_summary": "spec 新增 P0-11，server rules 未對應"
      }
    ]
  }
}
...
- `AC-P0-6-1` Given server rules 與 spec 差異超過純格式，When `analyze(check_type="governance_ssot")` 執行，Then 回傳 severity=red finding
- `AC-P0-6-2` Given `skills/governance/*.md` 缺少 reference-only 標示，When analyze 執行，Then 回傳 severity=yellow finding
- `AC-P0-6-3` Given 所有 SSOT 層同步，When analyze 執行，Then `findings` 為空陣列，`status=ok`
...
- `analyze(check_type="governance_ssot")` 為 Base Layer analyze tool 的新 check type，不是獨立 tool
- CI lint 的 spec↔server_rules diff 機制可用 pytest 測試或 GitHub Action，實作由 Architect 決定
...
5. `analyze(check_type="governance_ssot")` 已實作且至少跑過一次 full audit 且通過
