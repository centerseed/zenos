...
**與 v0.2 的差異**：v0.2 的 debrief 是一次性 streaming，使用者關閉面板後所有洞察消失。v0.3 要求 debrief 結果**持久展示**——寫回 CRM `ai_insights` 後，前端從 API 讀取並在活動時間軸和 AI 洞察面板中持久顯示。每次打開 deal 詳情頁，都能看到歷次商談的 AI 分析結果。
...
| 區塊 | 內容 | 資料來源 | 互動 |
|------|------|---------|------|
| 關鍵決策 | 歷次商談中做的所有決策，按時間排列 | 所有 `debrief` insights | 唯讀，可展開查看原始 debrief |
| 承諾追蹤 | 我方 + 客戶方的承諾事項，含截止日和完成狀態 | `commitment` insights | ☑ 可標記完成、可標記逾期 |
| 客戶顧慮 | 客戶歷次提出的疑慮和我方應對策略演進 | 所有 `debrief` insights 中的顧慮欄位 | 唯讀 |
| Deal 摘要 | 一段 AI 綜合摘要：目前狀態、風險、建議 | 最新一次 debrief 的下一步 + 階段建議 | 唯讀，隨新 debrief 自動更新 |

**面板位置**：Deal 詳情頁左側欄（與右側活動時間軸並列的雙欄佈局）。

**冷啟動**：尚無任何 debrief insight 時，面板顯示引導文字「完成第一次商談並記錄活動後，AI 洞察將在這裡累積」。
...
- **Acceptance Criteria**：
  ...
  - Given deal 有 1+ 筆 debrief insight，When 生成 briefing，Then 額外產出「累積洞察」區塊，包含歷次關鍵決策、未完成承諾、客戶核心顧慮
  ...
  - Given 第一輪 briefing 已生成，When 使用者送出第二輪以上追問，Then 前端必須沿用同一個 `conversationId`，並以 helper 的 `continue` / Claude session `resume` 模式續聊；不得重新開一個新的 `start` session 假裝是續聊
  - Given 對話進行中，When 使用者持續追問，Then 最多支援 8 輪對話（同行銷模組），超過後提示「請開啟新的準備 session」
  - Given 對話結束，When 使用者關閉面板，Then 對話歷史不持久化（同行銷模組），但 AI 生成的 briefing 會寫回 `crm.ai_insights`
  ...
  - Given 使用者在同一個 briefing 面板內持續追問，When helper session 仍存在，Then 每一輪都必須接到有效 SSE 回覆或明確錯誤；不得出現「送出成功但畫面無任何 AI 回應也無錯誤」的靜默失敗
  - Given 使用者在同一個 briefing 面板內持續追問，When helper 回傳 session not found / resume 失敗，Then 前端可重建一次新 session 後重送當前訊息，但整個過程必須對使用者可感知，且不得吞掉錯誤
  - Given briefing 生成完成，When AI 透過 Dashboard API 寫回 `crm.ai_insights`（`insight_type=briefing`），Then 前端收到 `done` SSE event 後立即顯示該 briefing，並可從 `GET /api/crm/deals/{id}/ai-entries` 再次讀回
  - Given helper 不可用或未啟動，When 使用者按「準備下次會議」，Then UI 顯示「請先啟動 Local Helper」引導（同行銷模組降級行為），CRM 其他功能不受影響
  - Given helper 在任一輪回傳 `stderr`、非 0 exit code、或 SSE `error` event，When 本輪對話失敗，Then UI 必須顯示可見錯誤訊息與可重試操作，且保留既有對話歷史
  - **圖譜驅動產品現況（引用 SPEC-cowork-knowledge-context）**：
  - `AC-CRM-BRIEF-20`: Given 使用者按「準備下次會議」，When helper 組裝 briefing context pack，Then 「產品現況」區塊的 context 透過 SPEC-cowork-knowledge-context 的圖遍歷 flow 取得（seed_entity = 該 deal 所連結的公司 entity 或關聯產品 entity）
  - `AC-CRM-BRIEF-21`: Given briefing 首輪生成，When UI 顯示，Then 對話視窗頂部顯示 SPEC-cowork-knowledge-context `AC-CKC-20~23` 定義的「已讀取 N 個節點 ▸」可展開清單
  - `AC-CRM-BRIEF-22`: Given 該 deal 連結的產品 entity 只有 L1，When briefing 生成，Then 行為遵循 `AC-CKC-40~42` L1-only fallback，briefing 仍可產出但標註「產品現況資訊有限」
  - **AI 對話視窗狀態（復用行銷模組狀態機）**：
  - Given briefing 生成流程，When 視窗狀態轉移，Then 遵循行銷模組的 7 狀態機（idle → loading → streaming → idle/error），UI 行為一致
  - **交付驗收邊界（新增，P0 release blocker）**：
  - Given 本功能要標記為可交付，When 進行 P0 驗收，Then 必須手動走完一次完整 happy path：開啟面板 → 產生第一輪 briefing → 送出至少一則追問並成功收到第二輪回覆 → 複製最新準備摘要
  - Given 本功能要標記為可交付，When CI 驗收，Then 至少有一條自動化 interaction/regression test 覆蓋「第一輪成功後第二輪必須 resume 成功」與「helper 失敗時 UI 顯示錯誤」兩條路徑
  - Given 上述任一條驗收未通過，When 準備交付或合併，Then 不得以「第一輪有出字」或「UI 已完成」視為完成

#### AI Debrief — 會後洞察與下一步（v0.3 更新：持久化展示）

- **描述**：使用者新增一筆會議類型的 Activity 並儲存後，前端自動組裝 debrief context pack 並透過 Local Helper bridge 呼叫本機 Claude CLI，AI 分析 Activity 摘要內容，以 SSE 串流產出結構化的會後洞察，寫回 CRM `ai_insights`。**v0.3 新增**：debrief 結果持久展示在活動時間軸和 AI 洞察面板中，不再隨面板關閉而消失
- **Acceptance Criteria**：
  - **核心功能**：
  ...
  - Given debrief 萃取出承諾事項，When 寫回 `crm.ai_insights`，Then 每個承諾事項包含：內容、owner（我方/客戶）、建議期限
  ...
  - **持久化展示（v0.3 新增）**：
  - Given debrief 已寫回 `crm.ai_insights`，When 使用者下次打開 Deal 詳情頁，Then 活動時間軸中該 Activity 下方顯示對應 debrief 摘要（關鍵決策 + 下一步），可展開查看完整 debrief
  - Given deal 有多筆 debrief insight，When 使用者查看 AI 洞察面板，Then 面板自動彙整所有 debrief 的關鍵決策、客戶顧慮（按時間排列），承諾事項獨立追蹤
  - Given 新的 debrief 生成完成，When 寫回 `crm.ai_insights` 成功，Then AI 洞察面板和活動時間軸自動刷新（不需使用者手動重新整理頁面）
  ...
  - Given debrief 生成完成，When AI 透過 Dashboard API 寫回 `crm.ai_insights`（`insight_type=debrief`），Then 承諾事項同時寫為獨立 `crm.ai_insights` row（`insight_type=commitment`）
...
- **描述**：Deal 詳情頁重構為雙欄佈局——左側 AI 洞察面板（始終可見）、右側活動時間軸。洞察面板從後端 API 讀取該 deal 的所有 AI insights（briefing / debrief / commitment），彙整為結構化的經營視角。活動時間軸中每筆會議/Demo/電話 Activity 可展開查看對應的 AI debrief 摘要
- **Acceptance Criteria**：
  - **AI 洞察面板**：
  - Given 使用者打開 Deal 詳情頁，When 頁面載入，Then 左側顯示 AI 洞察面板，從 `GET /api/crm/deals/{id}/ai-entries` 拉取資料
  - Given deal 有 1+ 筆 debrief insight，When 面板載入，Then 顯示「關鍵決策」區塊，按時間倒序列出所有 debrief 萃取的決策
  - Given deal 有 1+ 筆 commitment insight，When 面板載入，Then 顯示「承諾追蹤」區塊，分為「我方承諾」和「客戶承諾」兩組，每項顯示內容、截止日、完成狀態
  - Given deal 有 1+ 筆 debrief insight 含客戶顧慮，When 面板載入，Then 顯示「客戶顧慮」區塊，彙整歷次顧慮
  - Given deal 有最近一筆 debrief insight，When 面板載入，Then 顯示「Deal 摘要」區塊，內容為最新 debrief 的階段建議 + 下一步行動
  - Given deal 尚無任何 debrief insight，When 面板載入，Then 顯示引導文字「完成第一次商談並記錄活動後，AI 洞察將在這裡累積」
  - **承諾事項追蹤**（從 P1 提升到 P0）：
  ...
  - **活動時間軸內嵌 debrief**：
  - Given Activity（會議/Demo/電話）有對應 debrief insight，When 時間軸顯示該 Activity，Then Activity 下方顯示可展開的 AI debrief 摘要（預設收合，僅顯示「AI 分析 ▸」標籤）
  - Given 使用者點擊「AI 分析 ▸」，When 展開，Then 顯示該次 debrief 的關鍵決策和下一步行動（不是完整 debrief，是精簡版）
  - Given Activity 沒有對應 debrief insight（例如備忘類型），When 時間軸顯示，Then 不顯示「AI 分析」標籤
  - **後端 API**：
  - Given `GET /api/crm/deals/{id}/ai-entries`，When 呼叫，Then 回傳該 deal 在 `crm.ai_insights` 中的 briefing / debrief / commitment 資料，按 created_at 排序並分組
  - Given `PATCH /api/crm/commitments/{id}`，When 呼叫並傳入 `{status: "done"}`，Then 更新該 commitment insight 的 metadata.status，回傳更新後的資料
...
#### Deal Entity Projection Sync（L1 同步投影）

- **描述**：CRM 的公司 / 聯絡人 / Deal 主資料已橋接到 ZenOS entity。v0.3 補上 `deal` 的**當前進度投影同步**：不把 briefing / debrief / activity / commitment 逐筆鏡像成 ontology 節點，而是把 CRM 中最新、最有決策價值的摘要同步回**既有的 L1 `deal` entity**。目的不是把 ontology 變成 CRM timeline，而是讓知識圖譜能回答「這筆案子現在走到哪、卡在哪、下一步是什麼」。
- **同步範圍**：
  - `entity.summary`：簡短的人類可讀摘要，例如「需求訪談 · NT$500,000 · 最後互動 2026-04-15」
  - `entity.details.crm_snapshot`：結構化進度投影，至少包含：
    - `funnel_stage`
    - `amount_twd`
    - `last_activity_at`
    - `expected_close_date`
    ...
    - `latest_debrief_at`
- **邊界**：
  - briefing / debrief / commitment / activity 的**原始資料**仍以 `crm.ai_insights` 與 `crm.activities` 為準
  - ontology 只同步「當前有效投影」，不建立額外的 `crm_activity` / `crm_debrief` / `crm_commitment` entity
  - 不自動把 CRM 內容升格為 L2；若某種銷售知識具公司共識價值，需另走治理流程人工升格
- **Acceptance Criteria**：
  - Given CRM deal 已橋接為既有的 L1 `deal` entity，When 使用者更新漏斗階段，Then 同一個 `deal` entity 的 `summary` 與 `details.crm_snapshot.funnel_stage` 同步更新；不得建立新的 entity type
  - Given 使用者新增會議 / Demo / 電話 Activity，When Activity 儲存成功，Then `details.crm_snapshot.last_activity_at` 更新為最新互動時間
  - Given AI debrief 完成並成功寫入 `crm.ai_insights`，When 同步 projection，Then `details.crm_snapshot.latest_next_step`、`latest_customer_concerns`、`latest_debrief_at` 更新為最新有效值
  - Given 該 deal 有 open commitments，When 任一承諾被建立、完成或逾期，Then `details.crm_snapshot.open_commitments_count` 與 `overdue_commitments_count` 在同一次同步後反映最新數字
  - Given briefing 生成完成，When 存成 briefing insight，Then `details.crm_snapshot.latest_briefing_at` 更新，但 briefing 原文仍只存於 `crm.ai_insights`
  - Given 同步到 ontology，When 寫入 `deal entity`，Then 寫入內容僅限當前摘要投影，不得把完整 Activity 摘要、完整 briefing/debrief 長文原封不動塞入 `entity.summary`
  - Given 使用者在知識圖譜或其他讀 ontology 的入口查看該 deal，When 讀取 `deal entity`，Then 能從 `summary` 與 `details.crm_snapshot` 看出當前階段、最近互動、是否有未完成承諾、以及最近下一步
  - Given CRM 資料更新但投影同步失敗，When API 回應，Then CRM 寫入本身不可回滾失敗，但系統必須記錄錯誤並提供可重試路徑；不得 silently fail
  - Given projection sync 重跑，When 同一筆 deal 已存在對應的 ZenOS `deal` entity，Then 只更新既有 entity，不新增重複節點
...
| AI 產出 | 存放方式 | 說明 |
|---------|---------|------|
| Briefing | `crm.ai_insights` row | `insight_type = "briefing"`，每次生成存一筆，保留歷史 |
| Debrief | `crm.ai_insights` row | `insight_type = "debrief"`，跟隨 Activity 一一對應 |
| 承諾事項 | `crm.ai_insights` row | `insight_type = "commitment"`，含 owner / deadline / status（**P0 可追蹤**） |
| Follow-up 草稿 | Debrief metadata 的子欄位 | 不獨立存放，屬於 debrief 的一部分 |
...
| 同步目標 | 存放方式 | 說明 |
|---------|---------|------|
| Deal 當前進度投影 | 既有 ZenOS `deal` entity | 更新 `summary` + `details.crm_snapshot` |
| Briefing / Debrief / Commitment 原文 | **不同步為 ontology entry** | 原文與 metadata 以 `crm.ai_insights` 為唯一真相來源 |
| 相似案例、結案回顧的可複製知識 | P1/P2 視情況寫回 ontology | 僅限跨時間、跨 deal 有公司共識價值的精煉知識 |

### AI Entries 的前端讀取路徑（v0.3 新增）

v0.2 只定義了 AI 產出的寫入路徑，但**沒有定義前端如何讀取和展示**。v0.3 補齊：

| 前端元件 | 讀取 API | 顯示內容 |
|---------|---------|---------|
| AI 洞察面板 | `GET /api/crm/deals/{id}/ai-entries` | 顯示 saved briefings + 彙整 debrief 的關鍵決策、客戶顧慮 + commitment 追蹤 |
| 活動時間軸 | 同上（按 activity_id 匹配 debrief insight） | 每筆 Activity 下方可展開對應 debrief 摘要 |
| Briefing context pack | 同上 | 歷次 briefing 摘要 + debrief 摘要 + 未完成 commitment 作為 briefing 輸入 |
...
```json
{
  "briefings": [
    {
      "id": "insight-briefing-1",
      "created_at": "2026-04-15T15:44:00Z",
      "metadata": {
        "title": "會議準備 2026/04/15 下午03:44",
        "saved_at": "2026-04-15T15:44:12Z"
      },
      "content": "AI 產出的最新 briefing 摘要"
    }
  ],
  "debriefs": [
    {
      "id": "insight-debrief-xxx",
...
本 spec 基於 SPEC-crm-core 的資料模型。v0.3 新增 CRM schema table：`crm.ai_insights`，並要求 `crm.deals` 具備 `zenos_entity_id text` 欄位（同 companies/contacts 已有的 bridge pattern）。

AI 過程資料（briefing / debrief / commitment）以 CRM schema 為主，不直接寫成 ZenOS entry。ZenOS ontology 只同步既有 `deal` entity 的當前進度投影。
...
| **ADR-037 CRM Intelligence 架構** | 本 spec 的技術決策 | Deal type 命名、Local Helper 復用策略、AI 產出存放方式、context pack 結構、承諾提醒機制。若 ADR-037 仍記載「AI 產出存 ZenOS entry」，以本 spec + PLAN-crm-intelligence-v03 的修正版為準 |
...
| **SPEC-cowork-knowledge-context** | 引用 | Briefing 的「產品現況」「相似案例」區塊透過該 spec 的圖遍歷 flow 組裝 context pack。CRM 只負責定義 seed（deal 所屬公司/產品）與 briefing 的 target_fields；圖遍歷規則、透明度 UI、fallback 以 SPEC-cowork-knowledge-context 為 SSOT |
...
| 8 | Briefing / Debrief 觸發方式 | Local Helper bridge（方案 b） | 復用行銷模組架構（ADR-034）：前端 → Local Helper → 本機 Claude CLI → 生成 briefing/debrief → 寫回 `crm.ai_insights` → 前端顯示；必要時再同步精煉 projection 到既有 `deal` entity。不需要 API key、不需要 server-side LLM 接入。詳見 ADR-037 與 PLAN-crm-intelligence-v03 |
...
| 13 | Ontology 同步策略 | 只同步既有 L1 `deal` entity 的當前進度投影，不逐筆鏡像 CRM 過程資料 | 避免把知識圖譜退化成 CRM timeline；圖譜回答「現在這筆案子狀態如何」，CRM 表回答「每一筆過程發生了什麼」 |
