...
created: 2026-04-10
updated: 2026-04-23
depends_on: SPEC-ontology-architecture v2 §7.3, SPEC-entry-consolidation-skill, SPEC-governance-feedback-loop
---

# Feature Spec: Entry Distillation Quality

> **Layering note（2026-04-23）**：L2 entity 與 `EntityEntry` 的 canonical schema / lifecycle 在 `SPEC-ontology-architecture v2 §7.3`。本 SPEC 只管**寫入前的品質閘**（journal vs entry 邊界、蒸餾品質標準、可追溯性）；飽和後的 consolidation workflow 仍以 `SPEC-entry-consolidation-skill` 為權威。
...
- `SPEC-ontology-architecture v2 §7` 定義 L2 的定位與 entry 的 canonical schema / lifecycle
- `ADR-010-entity-entries` 定義 entries 的歷史資料模型決策（schema 已進主 SPEC）
...
- `docs/specs/SPEC-ontology-architecture.md v2 §7`（L2 + EntityEntry canonical）
...
衝突結論：無直接衝突，且本 SPEC 應獨立成文件，不應回填進主 SPEC §7。

原因：

- 主 SPEC §7 的責任是 L2 schema / lifecycle / entries sidecar 的 canonical 定義，不是定義蒸餾品質
- `ADR-010` 的責任是資料模型的歷史決策（schema 已進主 SPEC）
...
- **不重新定義 L2 三問**：L2 的分層標準仍以 `SPEC-ontology-architecture v2 §7.1` 為準
...
- PM：確認本 SPEC 作為獨立文件成立，不併入主 SPEC §7（舊 `SPEC-l2-entity-redefinition` 已於 2026-04-23 併入主 SPEC v2 §7；本 SPEC 因 scope 不同保持獨立）
