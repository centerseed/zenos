...
> 日期：2026-03-19（2026-03-20 架構對齊｜2026-03-21 North Star 更新｜2026-03-21 Ontology 架構定義｜2026-03-23 Entity/Project 分離 + Dashboard 多 View + 競品 + 市場定位 + Entity 分層模型 + 權限模型）
> 狀態：Canonical SSOT
> 審核定位：ZenOS 現行有效產品規格。若有新提案與此文件衝突，未經審核通過前，以本文件為準。

> 與 ontology 分層直接相關的現行章節：
> - Part 4 — Knowledge Ontology 技術路線
> - Part 7.2 — Entity 架構（2026-03-23 決策）
>
> 候選提案：
> - `docs/archive/specs/SPEC-ontology-layering-v2.md` 為待審 proposal，不自動覆蓋本文件
>
> 現行 active spec surface 見：
...
#### Entity 分層模型（2026-03-23 確立）

所有知識都是 entity，只是 type 不同。Entity 分三層：

```
第一層（基礎）    product          ← 每間公司一定有，最頂層的錨點
第二層（知識）    module           ← product 底下的知識聚合，業務領域拆分
第三層（應用）    document, goal, role, project  ← 按需生長，不是每間公司都需要全部
```

**全部都在同一個 `entities/` collection**，透過 `type` 和 `parent_id` 區分層級。知識地圖上每個節點都是一個 entity。

**Task 不是 entity。** Task 是行動，不是知識。它有自己的生命週期（backlog→done）、指派人、到期日、驗收條件——這些都不是知識的屬性。Task 透過 `linkedEntities` 連到 ontology，是 ontology 的消費者，不是 ontology 的一部分。
...
```
骨架層（Skeleton Layer）
  涵蓋：第一層（product）+ 第二層（module）+ 第三層的 goal / role / project
...
Step 1 — 讀 ontology（骨架層：product + module）
  「這間公司有 4 個產品、每個產品有哪些模組、它們之間的關係」
  → 建立公司全局理解

Step 2 — 根據任務需求，從 ontology 找相關 entity
  「這個任務需要 Paceriz 的定價策略」
  → 找到相關 module entity → 看它的 document entities 和 sources
  → 語意代理充當路由

Step 3 — 透過 entity 的 sources 或 document entity 的指引去讀實際文件
  → 拿到真正的資料，執行任務
```

Entity 就是公司知識的 SKILL.md——告訴 AI「這塊知識是什麼、為什麼存在、怎麼用、給誰用」。
...
```
Firestore
├── entities/          ← 所有知識節點（product, module, document, goal, role, project）
...
第二層（知識）    module           ← product 底下的業務領域拆分
...
## Part 7.3 — Dashboard 多 View 架構（2026-03-23 決策）
...
## Part 7.4 — 競品定位（2026-03-23 研究）

### ZenOS 的位置是空的

沒有產品同時滿足：AI 自動建構 ontology + SMB 導向 + 透過 MCP 給 AI agents 用。

### 最接近的競品

| 競品 | 差距 |
|------|------|
| **Graphlit** | 技術最近（Context Layer for AI Agents, 有 MCP）。但是開發者基礎設施，不是產品。沒有治理方法論 |
| **Tana** | 精神最近（知識圖譜，$25M 融資）。但是個人工具，要求搬進 Tana，沒有 MCP |
| **Glean** | 企業搜尋（$7.2B），不是知識圖譜。$50K 起跳，不會下沉 SMB |
| **Palantir** | Ontology 先驅，但只做結構化資料，$5M+/yr |
| **Notion/Guru** | 文件倉庫 + 搜尋，不是 ontology。反而是 ZenOS 的資料來源 |

### 護城河

技術不是護城河。**方法論才是**：四維標籤、漸進式信任、雙層治理、對話式建構。

Foundation Capital 2025 年底發表「Context Graphs 是兆元機會」論文，驗證賽道方向。窗口 12-18 個月。

---

## Part 7.5 — 台灣 SMB 市場定位（2026-03-23 洞察）

### 痛點不是「找不到文件」，是「流程跑不動」

台灣中小企業需要 ERP 等級的資訊流通，但買不起也不想導入傳統 ERP。

### 敘事

- **不說**：「我們做知識圖譜」「我們做 AI context 層」
- **說**：「你不用買 ERP，AI 就能幫你跑流程」「每個員工和 AI 都知道公司在幹嘛」

底層還是 ontology + MCP + AI agents，但老闆聽到的是流程變快了。

### ZenOS 的賭注

如果未來是 Copilot 模式（人問 AI 答）→ Glean 贏
如果未來是 Agent 模式（AI 自己做事）→ Glean 不夠，需要 ZenOS

ZenOS 賭 agent 時代。

---

## Part 7.6 — 權限模型（2026-03-23 設計）
...
**第 1 層不需要 ZenOS 管。** 員工點 entity.sources 的連結跳到 Google Drive 時，Drive 自己的 ACL 會擋。ZenOS 只需要用公司級 credential 讀取文件建 entity。

**第 2 層是核心。** 誰能看到哪些 entity、誰能修改 ontology。

**第 3 層是第 2 層的投射。** MCP token 和 Dashboard 登入都查同一套權限。
...
agent          authorizedEntityIds 範圍內讀寫（現有 partner model）
```

Entity 加 `visibility` 欄位：
...
`restricted` 的 entity 只有 admin 看得到。用於：薪資系統、合約、人事評估等。

### 不做的事（Phase 0）

- ❌ 細粒度 RBAC（per-entity per-member ACL）
- ❌ 繼承式權限（product restricted → 子 module 也 restricted）
- ❌ 來源層權限同步（Drive ACL → entity visibility）

等有客戶真的需要再擴展。三個欄位（role, visibility, authorizedEntityIds）撐過 Phase 0-1。
...
- [x] Ontology entry 的最小可行欄位定義 → Entity 分層模型（三層）+ entity.sources + document 是 entity type 不是獨立 collection（Part 7.2）
- [ ] 神經層 → 骨架層異常推斷的規則設計（閾值、觸發條件）
- [x] SMB 場景下的檔案存取方式 → Adapter 分階段（手動→Git hook→GitHub webhook→Google Drive/Notion OAuth），Google Workspace 支援 domain-wide delegation（Part 7.7）
...
- [x] Entity 分層模型 → 三層：product（基礎）→ module（知識）→ document/goal/role/project（應用）。Task 不是 entity（Part 7.2）
- [x] Entity 邊界定義 → 三個判斷標準：跨時間存活、有四維可描述、能當錨點（Part 7.2）
- [x] Document 的資料模型 → Document 是 entity(type="document")，不是獨立 collection。低價值文件用 entity.sources（Part 7.2）
- [x] Entity 用語規範 → UI 用「節點」或具體 type 名稱，不出現 entity/ontology（Part 7.2）
- [x] 權限模型 → Phase 0 三角色（admin/member/agent）+ entity.visibility（public/restricted）（Part 7.6）
- [x] Adapter 可行性 → GitHub/Google Drive/Notion/Confluence 確認可行，Google Workspace 支援 domain-wide delegation（Part 7.7）
- [ ] 權限模型：細粒度 RBAC 的觸發時機（哪個客戶場景需要 per-entity ACL）
- [ ] Adapter：Google Drive OAuth 的 Google 審核流程與時間線
- [ ] Entity.sources：`/zenos-capture` 掃目錄時自動填入 sources 的實作規格
...
*2026-03-23 加入 Part 7.2 Entity/Project 分離：Project 不是 Entity，是工作容器。Phase 0 用 product entity + owner，Phase 1 加 projects collection（ADR-006）*
*2026-03-23 加入 Part 7.3 Dashboard 多 View 架構：同一份 ontology 的不同投影（專案/知識地圖/任務）。三欄佈局（Palantir 啟發）。UI 不出現 entity/ontology 用語。Dashboard 5 份 spec 合併為 SPEC-dashboard-v1.md*
*2026-03-23 加入 Part 7.4 競品定位：ZenOS 位置空白（AI 自動 ontology + SMB + MCP），Graphlit/Tana 最近但差異明確，Foundation Capital 驗證賽道*
*2026-03-23 加入 Part 7.5 台灣 SMB 市場定位：ERP 角度切入，「不用買 ERP，AI 就能幫你跑流程」*
*2026-03-23 重寫 Part 7.2 Entity 架構：Entity 分層模型（三層）、邊界定義（三個判斷標準）、Document 是 entity type 不是獨立 collection、entity.sources 取代神經層獨立 collection、Task 不是 entity、Entity 用語規範（UI 用「節點」不用 entity）、知識地圖作為差異化展示策略*
*2026-03-23 更新 Part 4 雙層治理架構：骨架層=Layer 1+2，神經層=Layer 3 document entity。演化路線改為 MCP+Firestore → Adapter 自動化 → 全自動治理*
*2026-03-23 加入 Part 7.6 權限模型：三層權限（來源層/Ontology層/消費層），Phase 0 三角色（admin/member/agent）+ entity.visibility*
*2026-03-23 加入 Part 7.7 Adapter 可行性確認：GitHub/Google Drive/Notion/Confluence 整合方式、Google Workspace domain-wide delegation、漸進式授權與漸進式信任的關係*
