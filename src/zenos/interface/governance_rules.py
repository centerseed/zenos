...
## 關聯語意動詞（verb）
建立 relationship 時，`verb` 欄位必填（可選填但強烈建議）。
- 格式：2-5 字中文動詞，例：校準、觸發、驅動、限制、依賴、啟用、支撐
- `type` 是結構分類（impacts/enables），`verb` 是語意描述（「怎麼影響」）
- 若 write response 回傳 `suggested_verbs`，選最準確的或自行填寫更精確的動詞
- 沒有 verb 的關聯會在治理評分中扣分

## 影響鏈（impact_chain）
`get(collection="entities", id=...)` 回傳中包含 `impact_chain`：
多跳 BFS 遍歷，格式 `[{from_name, verb, to_name}, ...]`
...
## 關聯語意動詞（verb）
建立 relationship 時，`verb` 欄位強烈建議填寫：
- 格式：2-5 字中文動詞，例：校準、觸發、驅動、限制、依賴、啟用、支撐、規範
- `type`（impacts/enables/depends_on）是結構分類；`verb` 是語意描述（「怎麼影響」）
- 同一個 `type` 可能代表完全不同的語意，verb 才是真正的知識載體
- write response 回傳 `suggested_verbs` 時，選最準確的或自行填更精確的動詞
- 沒有 verb 的關聯在治理評分中會被標記為「語意不完整」

## 影響鏈（impact_chain）
`get(collection="entities", id=...)` 回傳中包含 `impact_chain` key：
BFS 多跳遍歷出邊，最多 5 跳，格式 `[{from_id, from_name, verb, type, to_id, to_name}]`
...
### 為什麼要填 verb
`type` enum（impacts/enables/depends_on）是結構分類，告訴你方向但不告訴你語意。
`verb` 是 2-5 字中文動詞，是真正的知識載體：
- 「跑步科學指標 [校準] Race Prediction」vs「跑步科學指標 [觸發] 行銷定位策略」
- 兩者 type 都可能是 impacts，但語意截然不同
- 無 verb 的知識地圖是「有骨架沒血肉」，agent 無法推理語意

### 填寫規則
- 建立 relationship 後，response 中有 `suggested_verbs`：選最準確的或自行填寫
- 用 `write(collection="relationships", id=existing_id, data={verb: "校準"})` 補填 verb
- 常用動詞參考：校準、觸發、驅動、限制、依賴、啟用、支撐、規範、定義、強化、約束
- 無法找到合適動詞 → 可能代表這條關聯本身語意不清，考慮重新審視

### 治理評分影響
- 有關聯但 verb 全空 → 治理報告標記「語意不完整」
- partner 整體 verb 填寫率 < 50% → 整體健康摘要標記為待改善
...
格式：`[{from_id, from_name, verb, type, to_id, to_name}, ...]`
...
```
impact_chain: [
  {from_name: "訂閱定價模型", verb: "校準", to_name: "Race Prediction"},
...
- **index**：文件本身是多個 source 的索引（如「產品文件集」包含 SPEC + DESIGN + TEST）
- 升級觸發：當 single 文件累積 3+ sources 且涵蓋 2+ 不同 doc_type → 建議升級為 index
- index 文件在 Dashboard 按 doc_type 分組顯示 sources
...
2. 已有文件 entity？→ 加為新 source（走 write per-source 操作）
3. 無既有 entity？→ 判斷類別（11 類），建新 document entity
4. 加完 source 後：sources 3+ 且 doc_types 2+ → 建議升級 doc_role 為 index
...
2a. 找到既有 document entity
    ...
    → 檢查 sources 數量 + doc_type 多樣性
    → 3+ sources && 2+ doc_types → 建議升級 doc_role 為 index
    ↓
2b. 無既有 entity
...
### 概念
- **single**（預設）：文件有一個主要 source
- **index**：文件是多個 source 的索引/集合
