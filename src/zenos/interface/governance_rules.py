...
## MCP write 最小格式
建立 L2 必須提供 `layer_decision`；缺少會回 `LAYER_DECISION_REQUIRED`。
...
write(collection="entities", data={
    ...
    "name": "清楚的 L2 名稱",
    "summary": "跨角色可讀的公司共識摘要",
    "tags": {"what": ["核心概念"], "why": "...", "how": "...", "who": ["PM", "Architect"]},
    "layer_decision": {
        "q1_persistent": True,
        "q2_cross_role": True,
...
- 新建 L2 一律為未確認狀態：`status="active"` + `confirmed_by_user=false`
- 未確認 → confirmed 必須走 confirm 工具，且需通過 impacts 驗證
- 沒有 impacts 的候選概念，應降為 L3 document 或 sources

## 生命週期
Draft：`status=active` + `confirmed_by_user=false`（新建預設）
Confirmed：`status=active` + `confirmed_by_user=true`（三問通過 + ≥1 具體 impacts）
Stale：`status=stale` + `confirmed_by_user=true`（impacts 目標失效或過時）
...
## MCP write 最小格式
建立 L2 必須提供 `layer_decision`，缺少會被 server reject 為 `LAYER_DECISION_REQUIRED`。
...
## 硬規則與生命週期
- 新建 L2 一律為未確認狀態：`status="active"` + `confirmed_by_user=false`
- write 不可直接把新 L2 設為 `confirmed_by_user=true`
- 未確認 → confirmed 必須走 confirm 工具，且需通過 impacts 驗證
- legacy `status="draft"` 只為舊資料相容；新建 L2 不再使用
...
- 未確認 → confirmed 必須有 ≥1 有效 impacts
...
- product_id: 歸屬的 L1 entity ID（任何 type 只要 level=1 且無 parent 皆可）；唯一 ownership SSOT（2026-04-22）
...
## 2026-04-22 Task Ownership SSOT
- 三條繩子分開：`product_id` = 歸屬、`plan_id`/`parent_task_id` = 編組、`linked_entities` = 知識關聯
- `product_id` 是唯一 ownership SSOT；`project` 字串只做顯示派生，不再是歸屬來源
- `linked_entities` 禁止含 L1 entity；若 caller 傳入，server 會 strip 並回 warning `LINKED_ENTITIES_PRODUCT_STRIPPED`
- caller 同時傳 `project` 字串與 `product_id` 時，以 `product_id` 為準；`project` 字串僅作 legacy 顯示派生使用
- subtask 的 `product_id` 必須等於 parent 的 `product_id`；task 有 `plan_id` 時也必須等於 plan 的 `product_id`
...
**OWNERSHIP_SSOT_PRODUCT_ID**：`product_id` 是唯一歸屬來源；`project` 字串不再代表 ownership。
- 沒傳 `product_id` 且 legacy `project` / `partner.defaultProject` 也無法解析 → `error_code=MISSING_PRODUCT_ID`
- `product_id` 指向不存在 entity 或非 L1 entity（level ≠ 1 或有 parent） → `error_code=INVALID_PRODUCT_ID`
