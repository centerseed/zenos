...
- product_id: 歸屬的 L1 product entity ID；唯一 ownership SSOT（2026-04-22）
...
- `linked_entities` 禁止含 product entity；若 caller 傳入，server 會 strip 並回 warning `LINKED_ENTITIES_PRODUCT_STRIPPED`
- caller 同時傳 `project` 字串與 `product_id` 時，以 `product_id` 為準；字串只作 legacy hint，server 可回 warning `PROJECT_STRING_IGNORED`
...
- `product_id` 指向不存在 entity 或 type ≠ product → `error_code=INVALID_PRODUCT_ID`
...
- Caller 傳 `project` 字串但與 product entity.name 不一致 → 以 `product_id` 為準 + warning `PROJECT_STRING_IGNORED`
