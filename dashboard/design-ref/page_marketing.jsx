// ZenOS v2 · Marketing — list + detail (second-layer Campaign workspace)
const { useState: useMkS } = React;
...
function InkMktPage({ t }) {
  const [openId, setOpenId] = useMkS(null);
  if (openId) return <InkMktDetail t={t} campaign={MKT_CAMPAIGNS.find(c => c.id === openId)} onBack={() => setOpenId(null)} />;
  return <InkMktList t={t} onOpen={setOpenId} />;
}

function InkMktList({ t, onOpen }) {
  const { c, fontHead, fontMono, fontBody } = t;
  return (
    <div style={{ padding: "40px 48px 60px", maxWidth: 1600 }}>
      <Section t={t} eyebrow="GROWTH · 行銷" title="行銷" en="Marketing"
      ...
      <div style={{ background: c.surface, border: `1px solid ${c.inkHair}`, borderRadius: 2, overflow: "hidden" }}>
        ...
        {MKT_CAMPAIGNS.map((k, i) => {
          const st = MKT_STATUS[k.status];
          return (
            <button key={k.id} onClick={() => onOpen(k.id)} style={{
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: c.inkMuted, marginTop: 3, display: "flex", gap: 8 }}>
                  <span>{k.channel}</span><span style={{ color: c.inkFaint }}>·</span>
                  <span>{k.owner}</span><span style={{ color: c.inkFaint }}>·</span>
...
// ─── Campaign detail (second layer) ─────────────────────────────────────
function InkMktDetail({ t, campaign, onBack }) {
  const { c, fontHead, fontMono, fontBody } = t;
  const [stage, setStage] = useMkS(campaign.stage);
  const st = MKT_STATUS[campaign.status];
