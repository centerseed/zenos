...
export function CopilotChatViewport({
  ...
  return (
    <div className="flex h-full flex-col gap-3 overflow-y-auto rounded-2xl border border-border/20 bg-card/30 p-4">
      {!hasContent ? (
        // Empty state
        <div className="m-auto max-w-md text-center">
          <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl border border-border/20 bg-card/50">
        ...
        <>
          {messages.map((msg, i) => (
            ...
            >
              {msg.role === "user" ? (
                <div className="max-w-[85%] rounded-xl border border-primary/20 bg-primary/10 px-4 py-3 text-sm text-foreground shadow-sm">
                  {msg.content}
                </div>
              ) : msg.role === "assistant" ? (
                <div className="max-w-[85%] rounded-xl border border-border/25 bg-secondary/35 px-4 py-3 text-sm text-foreground shadow-sm">
                  <MarkdownRenderer content={msg.content} className="text-sm text-foreground space-y-1" />
                </div>
              ) : (
                // system
                <div className="max-w-[85%] rounded-xl border border-border/15 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
                  {msg.content}
                </div>
              )}
            </div>
