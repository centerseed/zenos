...
> 改了 SQL schema → 確認 migration 已在 DB 執行。改了後端 → 部署 Cloud Run。改了前端 → 部署 hosting。

不能只部署一層，漏掉有變更的其他層。

> 📛 歷史教訓：SQL cutover 部署時 asyncpg 不支援 Cloud SQL socket URL 格式，上線後全部 401。
...
開 ZenOS task 是架構決策的一部分，不是形式上的紀錄。完整規範見 `docs/specs/SPEC-task-governance.md`。
...
```
1. search(collection="tasks", status="backlog,todo,in_progress,review,blocked") 去重
2. 確認是 task，不是 spec / blindspot / doc update
3. 選 1-3 個最合適的 linked_entities（先少掛，不要亂掛）
4. 寫出單一 outcome 的 title（動詞開頭）
5. 補齊 description（背景/問題/期望結果）與 acceptance_criteria（2-5 條可觀察條件）
...
|------|------|
| `title` | 動詞開頭，單一行動邊界 |
| `description` | 含背景、問題、期望結果（三件事缺一不可） |
| `acceptance_criteria` | 2-5 條可觀察條件，非過程性步驟或 roadmap 願景 |
| `linked_entities` | 1-3 個，掛最直接受影響節點，非湊數；找不到對應節點時標注 `[Ontology Gap: ...]` |
| `priority` | 不傳則由 server 推薦；有明確商業理由才覆蓋 |
| `status` | 建票只用 `backlog` 或 `todo`，不假設 `in_progress` / `done` |
| `assignee` | 直接填入，或在 description 記錄預期 owner + 指派條件；禁止 owner 未定且無指派條件 |
| `result` | 進入 review 前必須填，或在 description 末尾追加 `Result:` 區塊 |

### linked_entities 掛法原則

**類型 A（單點修補）**：掛直接受影響模組，不掛產品根節點
**類型 B（治理規則/流程）**：掛產品根節點 + 對應治理模組（+ MCP 模組，如涉及）
**類型 C（跨層架構）**：掛上位產品 + 最直接 module + 主要被 impacts 的治理節點

邊界判斷：主要驗收在「程式/資料行為修補」→ Type A；在「治理規則/文件契約變更」→ Type B；若同時成立且難以單票驗收 → 必須拆成兩張票。

### 常見反模式（建票前確認沒有）

- **孤兒票**：沒有 `linked_entities` + description 太短 + 缺 acceptance_criteria
- **假連結票**：`linked_entities` 填了但 description 沒提到這些節點
- **混合型票**：同一張票要求寫 spec + 實作 + migration + QA（需拆票）
- **提醒型票**：「記得之後看這個」，沒有 owner / 邊界 / 完成條件
- **重複票**：舊票沒 cancel 就再開一張；supersede 時在舊票 description 末尾附 `[Superseded by: TASK-XXX]` 並標 cancelled
...
> 📛 歷史教訓：2026-03-25。Spec 定義了 `list_all(type_filter)`，SQL 實作也支援了，但 Architect 拆任務時沒把「使用 type_filter」寫進 Done Criteria，導致 Developer 全部用 `list_all()` 無 filter 呼叫。同樣，`chat_structured` 的 `response_schema` 沒被正確傳給 Gemini，governance AI 從上線第一天就靜默失敗。
...
□ 確認要部署的所有層（hosting? cloud run? SQL migration?）
□ 環境變數 / secrets 已設定
...
□ 部署所有相關層（不是只部署一個）
    ...
    ./scripts/deploy_mcp.sh          # MCP Server (Cloud Run)
...
4. **不重複造輪子** — 有現成好工具就用
5. **不讓架構發散** — 回扣核心共識（PostgreSQL + MCP + 可抽換 AI 層）
6. **不過度設計** — YAGNI，現在不需要的彈性不加

### 架構共識

- **資料層**：PostgreSQL（Cloud SQL，schema `zenos`）
- **Agent 介面**：MCP 格式
...
| 情境 | 做什麼 |
|------|--------|
| 新技術決策 | 開新 `ADR-{序號}-{slug}.md`，序號全域唯一不重用 |
| 新實作設計 | 開新 `TD-{spec-slug}.md`（或 `TD-{spec-slug}-{layer}.md`） |
| 已 Accepted ADR 有新決策 | **絕不改 ADR 正文**，開新 ADR 並在舊 ADR 加 `superseded_by` |
| 實作對齊型更新（補 API、路徑、結果） | 直接更新既有文件 |
| 決策方向改變 | 開新文件，舊文件標 `Superseded` |

**文件生命週期變動（rename / archive / supersede）→ 必須同步 ontology：**

```python
# 封存舊文件時，更新對應 L3 entity 的狀態
mcp__zenos__write(id="{entity_id}", status="archived")

# 文件被取代時，建立追溯關係
mcp__zenos__write(id="{old_entity_id}", status="superseded",
                  details={"superseded_by": "{new_doc_id}"})
```

### ADR 格式

重大決策寫 ADR，存到 `docs/decisions/ADR-{3位數序號}-{topic}.md`：

```markdown
---
type: ADR
id: ADR-{序號}-{topic-slug}
...
技術設計存到 `docs/designs/TD-{spec-slug}.md`：
...
□ SQL 查詢都有 partner_id scope + ON CONFLICT WHERE partner_id guard
...
□ [文件治理] 我寫的 ADR / TD 有沒有 frontmatter（type / id / status / ontology_entity）？
□ [文件治理] 有沒有改了 ADR 正文（禁止）？還是有新決策應開新 ADR？
□ [文件治理] 文件 rename / archive / supersede 後，有沒有同步更新 ontology L3 entity？
□ [文件治理] ADR 序號有沒有跟現有文件衝突？（先 ls docs/decisions/ 確認）
```
