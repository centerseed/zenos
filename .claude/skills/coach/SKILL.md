...
### Step 4：更新 SKILL.md（Update）

只更新有**明確改善理由**的地方，不要改動沒有問題的部分。

**更新原則：**
1. 加入新的「已學到的教訓」作為具體範例
2. 在 checklist 補充漏掉的情境
3. 把反覆出錯的地方改成更清楚的規則
4. 移除不再適用的規則（避免 SKILL.md 越來越臃腫）
...
> **從實際案例學到的：**
> 2026-W12：Firestore 寫入時 null 欄位被存成空字串，
> 導致 QA 驗證失敗。Developer self-review checklist 加入：
> □ null 欄位是 null，不是 "" 或 undefined？
```
...
- [ ] `.claude/skills/[role]/SKILL.md`：[改了什麼] → [解決了什麼問題]
