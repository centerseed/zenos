...
- Claude 類：`.claude/mcp.json`
- Codex / ChatGPT：`.mcp.json`

不要把 `.claude/mcp.json` 當成所有平台共用路徑。
...
平台差異：
- Claude Code：看 `data.payload.claude_md_addition`
- Claude Web：看 `data.payload.project_instructions`
- Codex：看 `data.payload.agents_md_addition`
...
將 `data.payload.slash_commands` 中每個 key-value 寫入 `.claude/commands/` 目錄。
