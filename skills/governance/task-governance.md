...
- 內容本質上是新規格、新決策、新治理原則 → 先寫 spec / ADR
- 內容只是知識沉澱，沒有具體 owner / outcome / verification boundary → document update
- 內容只是執行者自己的短期 checklist，不需要跨人協作或驗收 → 不開票

### 判斷三問

1. 需要「誰來做、做到哪裡算完成」？→ 開 task
2. 需要「先定義規則或方向」？→ 先寫 spec / ADR
3. 系統看到一個缺口但還沒有方案？→ 先記 blindspot，再視情況轉 task
...
- blindspot → 可產生 task
- spec / design → 可產生 implementation task
- task 完成 → 可能反寫 document / blindspot / entity 狀態
...
1. **單一主要 outcome**：不混多個可獨立驗收的結果
2. **單一主要 owner**：有一個主要 assignee 或可明確指派的責任落點
3. **單一驗收邊界**：2-5 條 AC 即可判斷是否完成
...
當 task 涉及 draft 文件審核時，必須遵循以下半自動流程。
...
| `owner` | 最終確認文件是否離開 draft |

### 狀態流

```
draft（文章）→ todo/in_progress（審核 task）→ review（待 owner）
  → confirmed（文章離開 draft）
  → 或退回循環（owner 退回 → editor 修正 → 重新 review）
```

### 強制規則

1. 新 draft 文章必須對應至少一張 open 審核 task
2. 審核 task 必須有責任落點（優先用 `assignee_role_id`），不得無 owner
3. reviewer 送審前必須在 `result`（或 `Result:` 區塊）提供可驗收輸出
...
reviewer 的 `result` 或 `Result:` 區塊必須包含：

1. **審核摘要**：可讓 owner 快速判斷的重點整理
2. **審核結論**：「建議通過」或「建議退回」
3. **文章連結與審核證據連結**
4. **必修項目**（若需修改）：列出 editor 必須處理的項目清單
...
本 skill 治理 Task（Action Layer）。以下面向由其他 spec 權威定義，本 skill 僅引用、不重定義：

| 治理面向 | 權威文件 | 本 skill 行為 |
|---------|----------|--------------|
| L2 概念升級 / impacts gate | `SPEC-l2-entity-redefinition` | 僅引用 |
| 文件分類 / frontmatter / supersede / archive | `SPEC-doc-governance` | 僅引用 |
| Task 建票品質 / 粒度 / 去重 / 驗收 | `SPEC-task-governance` | 完整執行 |

### 衝突仲裁規則

1. 依憲法（`docs/spec.md`）第二節第⑥維度的通用仲裁順序處理
2. 本 skill 不得覆寫 L2 升降級規則
3. 本 skill 不得覆寫 L3 文件生命週期與 sync contract
4. 若 task 的 `linked_entities` 指向某 L2，而該 L2 狀態與 task 預期矛盾（例如要修改一個已 stale 的 L2）→ **先處理 L2 狀態，再執行 task**
5. Plan 完成判定不得覆蓋 task 逐張驗收結果
6. Task 驗收標準（AC）以 `SPEC-task-governance` 為準，Plan 的 `exit_criteria` 不得放寬 task 驗收
...
| `goal` | 計畫目標（一句話） |
| `owner` | 計畫責任人 |
| `entry_criteria` | 何時可啟動 |
| `exit_criteria` | 何時可結案 |
| `status` | 計畫狀態（`active` / `blocked` / `done`） |

### Task 在 Plan 下的擴充欄位

| 欄位 | 必填/選填 | 說明 |
|------|----------|------|
| `plan_id` | 必填 | 所屬 plan 識別 |
| `plan_order` | 必填 | 同一 plan 內的執行順序（整數，從小到大） |
| `depends_on_task_ids` | 選填 | 明確前置依賴（用於非線性流程） |

### 行為規則

- 領到 task 後，必須拉出同 plan 的所有 task，確認順序與依賴是否滿足
- 前置 task 未完成時，不得把後續 task 推進到可執行狀態
- 同一 plan 內不得存在衝突順序（重複 `plan_order` 且語意互斥）
- 缺少順序資訊的 plan task，不得自動派發給 execution agent
- Plan 不可直接派工，Task 才是唯一可 claim 的執行單位
- Plan 完成判定不得覆蓋 task 逐張驗收結果
- Plan 完成時必須提供彙總證據（至少包含完成範圍、未完成項、風險與後續建議）
